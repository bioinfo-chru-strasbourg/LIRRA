...
def test_dialog_user():
    choix = input("If you execut this pytest all results directory are overwrite, do you want continue ? [y/n] \n")
    ...
    if choix.lower() == "y":
        ...
        print("Thanks write Y for agree and continue this pytest or N for quit and save your previous results")
        test_dialog_user()
...
def test_run_snakefile():
    ...
    if os.path.exists(com.snp_file):
        ...
        sp.run([
            "mv",
            com.snp_file,
            com.config_directories_path+"/snp_data_previous.tsv"
        ])

        sp.run([
            "cp",
            com.test_path+"/unit/snp_data.tsv",
    ...
    if os.path.exists(com.config_directories_path+"/config.yaml"):
        ...
        sp.run([
            "cp",
            com.test_path+"/unit/config.yaml",
            com.config_directories_path
        ])
    else :
        #export list fam test
    ...
    #run snakemake
    os.system("snakemake -c10 --use-conda --conda-frontend conda --directory /home1/BAS/hameaue/LIRRA/workflow/ -s /home1/BAS/hameaue/LIRRA/workflow/Snakefile")
...
def test_rule_create_file_map_first():
    ...
    sp.check_output([
        "cmp",
        com.integration_path+"/create_file_map_first/expected/plink_with_zero.map",
