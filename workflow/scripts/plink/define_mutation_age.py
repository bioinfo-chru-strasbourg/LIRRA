...
class create_Mutation_script:
    ...
    def load_config_args(self):
        with open(os.path.join(os.path.dirname(__file__),"..","..","..","config","config.yaml"), "r") as file:
            ...
            self.chance_sharing_correction= str(prime_service["params"]["correction_dating"]).upper()
    ...
    def append_algo(self):
        lines=[]
        with open(os.path.join(os.path.dirname(__file__),"mutation_script_cut.R"), "r") as script_r_read :
